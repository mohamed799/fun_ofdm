...
namespace fun {
     ...
     */
    transceiver::transceiver(void (*callback)(std::vector<std::vector<unsigned char> > packets), double freq, double samp_rate, double rx_gain, std::string device_addr) :
        transceiver(callback, usrp_params(freq, samp_rate, 20, rx_gain, 1.0, device_addr))
    {
    }
     ...
     */
    transceiver::transceiver(void (*callback)(std::vector<std::vector<unsigned char> > packets), usrp_params params) :
        m_usrp(params),
     ...
     */
    void transceiver::receiver_chain_loop()
    {
        while(1)
        {
